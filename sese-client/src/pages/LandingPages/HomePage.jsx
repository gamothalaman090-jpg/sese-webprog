...
const HomePage = () => {
    ...
    useEffect(() => {
        fetchArticles()
            .then(({ data }) => {
                ...
                setFeatured(articles.slice(0, 2));
            })
            .catch(console.error);
    }, []);
    ...
    return (
        <div className="flex w-full flex-col">
            ...
            {/* §02 FEATURED ARTICLES — live from DB */}
            <section className="border-b-2 border-[#0a0a0a] px-4 sm:px-6 lg:px-8 py-12">
                <div className="mx-auto max-w-6xl">
                    ...
                    {featured.length === 0 ? (
                        ...
                        <div className="grid sm:grid-cols-2 border-2 border-[#0a0a0a] divide-x-2 divide-[#0a0a0a]">
                            {[1, 2].map(n => (
                                <div key={n} className="flex flex-col">
                                    <div className="h-52 bg-[#e4e4e7] animate-pulse border-b-2 border-[#0a0a0a]" />
                                    <div className="p-6 flex flex-col gap-3">
                                        <div className="h-2 w-32 bg-[#e4e4e7] animate-pulse" />
                                        <div className="h-5 w-48 bg-[#d4d4d8] animate-pulse" />
                                        <div className="h-2 w-full bg-[#e4e4e7] animate-pulse" />
                                        <div className="h-2 w-3/4 bg-[#e4e4e7] animate-pulse" />
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="grid sm:grid-cols-2 border-2 border-[#0a0a0a] divide-x-2 divide-[#0a0a0a]">
                            {featured.map((p, i) => (
                                <article key={p._id} className="flex flex-col">
                                    ...
                                    <div className="p-6 flex flex-col flex-1">
                                        <p className="text-[10px] font-semibold uppercase tracking-[0.35em] text-[#71717a] mb-2">
                                            {String(i + 1).padStart(2, '0')} — {p.type} · {p.stack}
                                        </p>
                                        ...
                                        <Button className="mt-5" to={`/articles/${p._id || p.id}`}>View Project</Button>
                                    </div>
                                </article>
                            ))}
                        </div>
